import {beginnerTitles} from '../beginner';
...
export function exercises(language:Language,level:Level,units:Unit[],variant=0):Exercise[]{
  ...
  return units.flatMap((u,i)=>{
    ...
    return (['predict','complete','repair'] as const).map((kind,k)=>({
      id:`${language}-${level}-${u.id}-${kind}`, language,level,unit:u.id,title:level==='beginner'?(beginnerTitles[u.id]??u.title):u.title,explanation:u.explanation,
